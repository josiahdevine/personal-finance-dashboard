...
const ErrorFallback = ({ 
  ...
  // Check if error is an API error by checking properties
  const isApiError = error && 'status' in error && 'data' in error;
...
class ErrorBoundaryClass extends Component<Props & { isDarkMode: boolean }, State> {
  ...
  isApiError(error: unknown): boolean {
    // Safer check for API error without importing ApiError
    return error !== null && 
           typeof error === 'object' && 
           'status' in error && 
           'data' in error &&
