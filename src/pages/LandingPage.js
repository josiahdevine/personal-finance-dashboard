...
try {
  // Check if components are already registered to avoid duplicate registration
  const isRegistered = ChartJS.registry.controllers.get('line');
  if (!isRegistered) {
    ChartJS.register(
      CategoryScale,
      LinearScale,
    ...
    console.log('[ChartJS] Components already registered');
  }
} catch (error) {
  console.error('[ChartJS] Error registering components:', error);
}
